...
public final class Pedone extends Pezzo {
	...
	public boolean isMossaSpecialeValida(Cella start, Cella end, ArrayList<String> mosse) {
		...
		cellaPedone = Scacchiera.getCella(end.getX(), start.getY()); // Prendo la colonna della destinazione, e la
																		...
																		// cellaPedoneDaCatturare
		if (isPedone(cellaPedone) && cellaPedone.getPezzoCorrente().getColore() != this.colore) { // se la cella è
																									// occupata da un
																									// pedone di un
																									// colore opposto
			...
			if (this.colore == Colore.bianco) {
				...
				if ((start.getY() - 1 == end.getY()) && // Se la traversa di partenza + 1 è uguale a quella di
														// destinazione
						(Math.abs(start.getX() - end.getX()) == 1) && // E mi sto spostando in obliquo
						!end.isOccupato() && // E la cella di dest. non è occupata
						mosse.get(mosse.size() - 1).equals(pedoneAvversarioNero) // E l'ultima mossa è quella prevista
																					// per l'e.p.
				)
...
>>>>>>> 1faf50278d4332cf67f75e46a609df83ca7400d9
	 * @param s
	 * @param g
	 * @return
	 */
		...
		if (mossa.matches(regexCattura)) { // Mossa di cattura in diagonale
			...
			if (g.getColore() == Colore.bianco) {
				...
				if (mossa.charAt(1) == '4' && // Se mi voglio spostare nella 4 traversa, devo determinare se voglio
					...
					variazione = -50; // Se la terza traversa non è occupata da un pedone allora vengo dalla 2°
										// traversa
