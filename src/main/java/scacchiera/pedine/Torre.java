...
private static String posizioneValidaRiga(final ArrayList<String> possibiliPosizioniRiga, final int eX, final int eY,
		final Colore colorepedineGiocatoreCorrente) {
	...
	if ((sX != eX || sY != eY) && cellaCorrente.isOccupato()
		...
		if (sY > eY) {
			...
			for (int i = sY - 1; i >= eY; i--) {
				...
				if (cellaCorrente.isOccupato() && i == eY && pezzoCorrente.getColore() != colorePezzoGiocatoreCorrente
						&& isMossaCattura) {
					return true;
		...
		if (sX > eX) {
			...
			for (int i = sX - 1; i >= eX; i--) {
				...
				if (cellaCorrente.isOccupato() && i == eX && pezzoCorrente.getColore() != colorePezzoGiocatoreCorrente
						&& isMossaCattura) {
					return true;
