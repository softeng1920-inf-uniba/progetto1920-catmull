...
public final class Re extends Pezzo {
	...
	if (!isReSottoScacco(end)) {
	    ...
	    } else if (start.getY() == end.getY()) { // sulla stessa colonna: + 1 a sinistra, -1 a destra
	...
	if (mossa.matches("R(x|:)[a-h][1-8]")) {
	    ...
	    if (Scacchiera.getNomePezzo(endX, endY) == "Vuota"
		    || Scacchiera.getCella(endX, endY).getPezzoCorrente()
	...
	for (int i = 0; i < Scacchiera.getNumeroColonne(); i++) {
	    for (int j = 0; j < Scacchiera.getNumeroRighe(); j++) {
		if (
			Scacchiera.getNomePezzo(i, j) != "Vuota"
     ...
     */
    private static boolean isPrimaMossaEffettuata(final ArrayList<String> storicoMosse, final int tipoArrocco) {
	...
	for (int i = 0; i < storicoMosse.size(); i++) {
		...
		if (((i % numGiocatori) == 0 && Turno.getGiocatoreInTurno().getColore() == Colore.bianco)
			|| ((i % numGiocatori) != 0 && Turno.getGiocatoreInTurno().getColore() == Colore.nero)) {
		    return true;
		}
	    }
	}
