...
public final class Controller {
	 ...
	 * e restituisce true se la mossa è valida ed è stata applicata altrimenti
	 * restituisce false
	 *
	...
	public static boolean mossaScacchi(final String mossaDaGiocare) {
		...
		if (tipoMossa == 2) {
			...
			if (Scacchiera.isRangeValido(Cella.startX(mossaEstesa), Cella.startY(mossaEstesa))
				...
				if (tipoMossa != EN_PASSANT) { // se di sicuro non è un en paasant
					if (isMossaValidaGlobale(mossaEstesa, tipoMossa)) {
						applicaMossa(cellaPartenza, cellaDestinazione, tipoMossa);
					} else {
						return false;
					}
				} else if (tipoMossa == EN_PASSANT || tipoMossa == 1) {
					// se può essere un en passant
	 ...
	 * se è una mossa semplice, 1 se è una mossa di cattura, 2 se è un arrocco e 3
	 * se è di sicuro un en passant
	 *
	 * @param cellaPartenza
	...
	/**
	 * Dopo che sono stati effettuati i vari controlli (se la mossa inserita Ã¨
	 * valida, se Ã¨ consentita ecc.), viene effettuato in base alla tipologia della
	 * mossa, il necessario per proseguire con il corretto funzionamento del gioco.
	 *
	 * L'applicazione della mossa non Ã¨ univoca per ogni comando inserito (vedi
	 * e.p.)
	 *
	 * @param cellaPartenza
	 * @param cellaDestinazione
	 ...
	 * @param mossaTorre
	 * @param tipoArrocco
	 * @return true se l'arrocco puÃ² essere effettuato, false se l'arrocco e' stato
	 ...
	 * @param destinazione
	 * @param tipoMossa
	 * @return isReProtetto: falso se il Re Ã¨ sotto scacco, vero altrimenti.
	 */
